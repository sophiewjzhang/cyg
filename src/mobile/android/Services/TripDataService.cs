...
namespace android.Services
{
    public class TripDataService : BaseDataService, ITripDataService
    {
        ...
        {
            var todaysTrips = await GetCachedAsync<TripFromTo>($"trip/{from.StopId}/{to.StopId}/{routeId}/{DateTime.Now.Date}");
        ...
        public async Task<IEnumerable<TripFromTo>> GetTripsFromTo(string routeId, DateTime date, Stop from, Stop to)
        {
            ...
            return await GetCachedAsync<TripFromTo>($"{AllTripsCacheKey}:{routeId}:{dateFormat}:{from}:{to}", $"trip/{from.StopId}/{to.StopId}/{routeId}/{dateFormat}");
        }
    }
}